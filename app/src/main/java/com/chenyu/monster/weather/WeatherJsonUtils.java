...
public class WeatherJsonUtils {
    ...
    public static List<WeatherBean> getWeatherInfo(String json) {
        ...
        if ("1000".equals(status)) {
            ...
            for (int i = 0; i < size; i++) {
                WeatherBean weather = getWeatherBeanFromJson(array.get(i).getAsJsonObject());
    ...
    private static WeatherBean getWeatherBeanFromJson(JsonObject jsonObject) {
        ...
        weatherBean.date = date;
        weatherBean.temperature = temperature;
        weatherBean.weather = weather;
        weatherBean.week = date.substring(date.length() - 3);
        weatherBean.wind = wind;
        weatherBean.imageRes = getWeatherImage(weather);
        return weatherBean;
    }
